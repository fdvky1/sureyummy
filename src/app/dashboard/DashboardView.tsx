...
import { getAIInsights, getBasicAnalytics } from "./actions"
...
export default function DashboardView({ stats, userName }: DashboardViewProps) {
    ...
    // Load basic analytics (KPI, insights) on mount
    useEffect(() => {
        async function fetchBasicAnalytics() {
            try {
                const result = await getBasicAnalytics()
                if (result.success && result.data) {
                    // Load everything except aiAnalysis and recommendations
    ...
    return (
        <div className="min-h-screen bg-base-200 p-6">
            <div className="max-w-7xl mx-auto">
                ...
                {/* AI Recommendations & Strategic Analysis Section (On-Demand) */}
                <div className="mt-8">
                    <div className="flex items-center justify-between gap-3 mb-6">
                        <div className="flex items-center gap-3">
                            <RiBrainLine className="w-8 h-8 text-primary" />
                            <div>
                                <h2 className="text-2xl font-bold">AI Recommendations & Strategic Analysis</h2>
                                <p className="text-sm text-base-content/70">
                                    {aiInsights?.aiAnalysis ? 'AI-powered recommendations and strategic insights' : 'Generate rekomendasi dan analisis strategis berbasis AI'}
                                </p>
                            </div>
                        ...
                        <button 
                            ...
                            className={`btn gap-2 ${aiInsights?.aiAnalysis ? 'btn-outline btn-sm' : 'btn-primary'}`}
                        >
                            {isLoadingAI ? (
                                <>
                    ...
                    {!aiInsights?.aiAnalysis && !isLoadingAI && (
                        <div className="card bg-base-100 shadow-xl border-2 border-dashed border-primary/30">
                            <div className="card-body text-center py-12">
                                ...
                                <h3 className="text-xl font-bold mb-2">AI Analysis Belum Di-generate</h3>
                                <p className="text-base-content/70 mb-4">
                                    Klik tombol "Generate AI Analysis" untuk mendapatkan rekomendasi dan analisis strategis
                                </p>
                                <div className="flex flex-wrap gap-2 justify-center text-sm text-base-content/60">
                                    ...
                                    <span className="badge badge-ghost">✓ Strategic Business Analysis</span>
                                    <span className="badge badge-ghost">✓ Risk Factors & Opportunities</span>
                                </div>
                            </div>
                        </div>
                    )}

                    {/* AI Recommendations & Strategic Analysis Content */}
                    {aiInsights?.aiAnalysis && !isLoadingAI && (
                        <>
                        {/* AI Recommendations */}
                        <div className="card bg-base-100 shadow-xl border border-base-300 mb-6">
